...
#List of usernames that it is okay to change
safe_user_list_text_file = open("safe_user_list.text", "r")
safe_user_list = safe_user_list_text_file.readlines()
safe_user_list_text_file.close()
									...
									if (len(userEmailAddresses.entry) > 0):
										...
										if (user_full_name==""):
											...
											if (isDefault=="true"):
												...
												if any(user in s for s in safe_user_list):
													...
													print "User not in list, but would otherwise be Creating Alias for user %s full name '%s' address %s" % (user, user_full_name, user_new_email)
